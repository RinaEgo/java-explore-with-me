...
public class EventServiceImpl implements EventService {
        ...
        if (updateEventAdminRequest.getStateAction() != null) {
            if (updateEventAdminRequest.getStateAction() == StateActionAdmin.PUBLISH_EVENT) {
                if (eventToUpdate.getState().equals(State.PENDING)) {
                    ...
                    throw new AlreadyExistsException("Ошибка. Событие можно публиковать, только если оно в состоянии ожидания публикации.");
                }
            }
            if (updateEventAdminRequest.getStateAction() == StateActionAdmin.REJECT_EVENT) {
                if (eventToUpdate.getState().equals(State.PUBLISHED)) {
                    throw new AlreadyExistsException("Ошибка. Событие невозможно отклонить, если оно уже опубликовано.");
                }
    ...
    public List<EventShortDto> getAllPublic(String text, List<Long> categoriesIds, Boolean paid, String rangeStart,
        ...
        statisticsClient.saveHit("ewm-service", request.getRequestURI(), request.getRemoteAddr(), LocalDateTime.now());
    ...
    public EventDto getByIdPublic(Long eventId, HttpServletRequest httpRequest) {
        ...
        statisticsClient.saveHit("ewm-service", httpRequest.getRequestURI(), httpRequest.getRemoteAddr(), LocalDateTime.now());

        Long views = statisticsClient.getStatisticsByEventId(eventId);
    ...
    private void updateEventByAdmin(UpdateEventAdminRequest event, Event eventToUpdate) {
        ...
        eventToUpdate.setCategory(event.getCategory() == null
                ...
                : categoryRepository.findById(event.getCategory()).orElseThrow(() -> new NotFoundException("Категория с id " + event.getCategory() + " не найдена.")));
    ...
    private List<EventDto> getEventDtoListWithAllParameters(List<Long> userIds, List<Long> categories,
        ...
        Map<Long, Long> viewStatsMap = statisticsClient.getSetViewsByEventId(eventIds);
